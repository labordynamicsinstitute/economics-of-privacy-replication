...
try;
%parameters
	n = 197040596; %Estimate of the 2012 US population age 16-64 based on ACS. See $A/data/raw/population_estimates in this archive.
	%see also  

	chi = 797; %size of sample space. 1000 quantiles of the income distribution.
	k = 318003; %size of the set of allowable queries. constructed here as the number of possible choices of two points in the histogram to allow "less than or equal to" and "greater than" queries
    %see code for ACS model.

	SWF_alpha_int = .7; %intercept for another level set of the SWF
	sum_p = 2;
	ybar = 9;
	cov_y_gamma = 0.083; %from analysis of FSS POS. See text for details.
	cov_y_eta = 0.040;    %from analysis of FSS POS. See text for details.
...
%plotting
	...
	axis square;
	set(gca,'FontSize',12);
	print('-f1','-deps','plannersprob_income');
	print('-f1','-djpeg','plannersprob_income');
	hold off;
clear all;
catch err; 
    err.message
