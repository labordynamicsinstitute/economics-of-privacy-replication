...
def main():
    ...
    eps_grid=[.0221] #for custom gird or single value
    ...
    #With each run of the program, new sample data will be appended to the output files unless file names are changed
        with open('ACS_dev_kl_measurements.csv','ab') as ofile:
            writer=csv.writer(ofile)
            writer.writerow(kl_measurements)
        with open('ACS_dev_max_error_measurements.csv','ab') as ofile:
            writer=csv.writer(ofile)
            writer.writerow(max_error_measurements)
        with open('ACS_dev_mse_measurements.csv','ab') as ofile:
            writer=csv.writer(ofile)
